...
public class BinarySearchTree
{
    ...
    {
    	return replace(x.toLowerCase(), y, root.getLeftChild());
    }
    ...
    private boolean contain(String x, Node p)
    {
    	if (p == null)
            return false;
        else if (x.equals(p.getInfo().word))
            return true;
        else if (cmp(x, p) < 0)
    ...
    private String find(String x, Node p)
    {
    	if (p == null)
            return "Not found!";
        else if (x.toLowerCase().equals(p.getInfo().word.toLowerCase()))
    ...
    private boolean replace(String x, String y, Node p)
    {
    	if(p == null) {
    		...
    		p.setInfo(new ElementType(x.toLowerCase(),y.toLowerCase()));
    		return true;
    	}
    	else if(cmp(x, p) < 0) {
